...
import { getSearchThemeList } from "@api/theme/themeApi";
export default {
  created() {
  ...
  methods: {
    currentChange(newPage) {
      ...
      this.getThemeVOListFromServer(this.searchContent,this.currentPage,this.pageSize);
    },
    getThemeVOListFromServer(searchContent, page, pageSize) {
      getSearchThemeList(searchContent, page, pageSize).then(data => {
        let result = data.data;
        this.themeVOList = result.themeVOList;
        this.themeCount = result.themeCount;
      });
    },
  ...
  computed: {
    isMultiPage() {
      return this.themeCount / this.pageSize > 1;
    },
    isNoTheme() {
      return this.themeCount === 0;
    }
  }
};
</script>
